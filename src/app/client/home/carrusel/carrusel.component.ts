...
export class CarruselComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  initializeCarousel(): void {
    if (!this.sliderContainer || !this.nextButton || !this.prevButton) {
  ...
  actualizarBookActual(): void {
    ...
    } else if (this.SliderDom.children.length > 0) {
      // Asignar el primer elemento como 'first' si no hay ninguno
  ...
  moveSlider(direction: string): void {
    ...
    if (direction === 'next') {
      // Animación simplificada usando CSS transitions
      ...
      setTimeout(() => {
        ...
        this.SliderDom!.style.transition = 'transform 0.8s cubic-bezier(0.4, 0, 0.2, 1)';
